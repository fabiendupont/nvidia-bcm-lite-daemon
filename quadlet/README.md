...
```bash
# Pull the BCM Agent image (update with your registry)
sudo podman pull ghcr.io/fabiendupont/nvidia-bcm-lite-daemon:1.0.0

# Or load from tar if pushing to registry is not available
sudo podman load -i bcm-agent-1.0.0.tar
```
...
```bash
# Pull new image
sudo podman pull ghcr.io/fabiendupont/nvidia-bcm-lite-daemon:1.0.1

# Update Quadlet file with new tag
sudo vi /etc/containers/systemd/bcm-agent.container
# Change: Image=ghcr.io/fabiendupont/nvidia-bcm-lite-daemon:1.0.1
