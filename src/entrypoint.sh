...
shutdown() {
    ...
    # Stop K8s labeler
    if [ ! -z "$K8S_PID" ] && kill -0 "$K8S_PID" 2>/dev/null; then
...
# Build cm-lite-daemon arguments
CM_ARGS=""

# Add host/port if not using config file
if [ ! -f "$CONFIG_FILE" ]; then
    CM_ARGS="$CM_ARGS --host $BCM_HOST --port $BCM_PORT"
fi

# Add certificate files
if [ -f "$CERT_FILE" ]; then
    CM_ARGS="$CM_ARGS --cert $CERT_FILE --key $KEY_FILE"
fi

if [ -f "$CA_FILE" ]; then
    CM_ARGS="$CM_ARGS --ca $CA_FILE"
fi

# Add config file if it exists
if [ -f "$CONFIG_FILE" ]; then
    CM_ARGS="$CM_ARGS --config $CONFIG_FILE"
fi

# Add node name
CM_ARGS="$CM_ARGS --node $NODE_NAME"

# Disable hostname check (certificate may not match hostname)
CM_ARGS="$CM_ARGS --disable-hostname-check"

# Enable debug if requested
if [ "$DEBUG" = "true" ]; then
    CM_ARGS="$CM_ARGS --debug"
fi
